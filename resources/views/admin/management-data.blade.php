               ...
               @foreach ($persons as $person)
                  <tr class="hover:bg-gray-50 transition-colors">
                     ...
                     <x-td class="text-center">
                        <span class="px-3 py-1 rounded-full text-xs {{ $person->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                           {{ $person->is_active ? 'Aktif' : 'Tidak Aktif' }}
                        </span>
                     </x-td>
                     <x-td>
                        <div class="flex gap-4 justify-center items-center">
                           <a href="{{ route('admin.edit', $person->id) }}" class="p-2.5 bg-dongker-sidilan rounded-lg text-white hover:opacity-90">
                           ...
                           <button 
                              type="button" 
                              class="p-2.5 bg-orange-sidilan rounded-md hover:opacity-90"
                              @click="
                                 showModal = true;
