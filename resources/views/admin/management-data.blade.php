...
@section('content')
   <div x-data="{ 
            ...
            <tbody>
               @foreach ($persons as $person)
                  <tr>
                     ...
                     <x-td class="text-center">{{ $person->is_active ? 'Aktif' : 'Tidak Aktif' }}</x-td>
                     <x-td class="flex gap-4 justify-center items-center">
                        <a href="{{ route('admin.edit', $person->id) }}" class="p-3 bg-dongker-sidilan rounded-lg text-white">
                        ...
                        <button 
                           type="button" 
                           class="p-3 bg-orange-sidilan rounded-md"
                           @click="
                              showModal = true;
      ...
      {{-- Letakkan Modal di Sini (Satu saja untuk semua data) --}}
      <x-modal-box />
   </div>
@endsection